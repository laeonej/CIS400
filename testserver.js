...
const PORT = 1111; // PORT of server
...
io.on('connection', client => {
    ...
    client.on('checkUserDetail', data => {
        ...
        for (var id in sockets) {
            if (sockets[id].mobile_number === data.mobileNumber) { //user currently playing on other browser
                userExists = true;
                break;
            }
        }
        if (!userExists) {
            ...
            for (var id in players) {
                if (id === data.mobileNumber) { //actual player is the same 
                    playerIdExists = true;
                    break;
                }
            }
